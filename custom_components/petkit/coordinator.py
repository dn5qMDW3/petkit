...
# NOTE: PetKit's API servers use SSL certificates signed by "Xcc Trust OV SSL CA"
# (a Chinese CA by Beijing Xinchacha). The root CA is Certum (Polish, widely trusted),
# but the intermediate "Xcc Trust" is NOT in Python's certifi bundle.
# curl trusts it via macOS Keychain, but aiohttp/Python may fail with
# SSLCertVerificationError. The HA aiohttp session handles this via system certs.
...
class PetkitDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self.current_devices: set[int] = set()
...
class PetkitMediaUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self.media_type: list[MediaType] = []
        self.event_type: list[RecordType] = []
        self.previous_devices: set[int] = set()
        self.media_table: dict[int, list[MediaFile]] = {}
        self.delete_after = 0
        self.media_path = Path()
        # Load configuration
        self._get_media_config(config_entry.options)

    def _get_media_config(self, options: dict[str, Any]) -> None:
    ...
    async def _async_update_data(
        ...
        for device_id in self.data_coordinator.current_devices:
            ...
            if isinstance(device, WaterFountain):
                LOGGER.debug(
                    "Updating bluetooth connection for device id = %s", device_id
                )
                self.hass.async_create_task(
